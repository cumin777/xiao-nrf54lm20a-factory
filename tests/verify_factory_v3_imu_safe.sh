...
rg -n "g_imu_text_report_active|g_imu_text_report_work|at_imu_text_report_work_handler" src/at_handler.c >/dev/null
rg -n "sensor_attr_set\\(|at_imu_set_sampling_freq" src/at_handler.c >/dev/null
rg -n "CONFIG_LSM6DSL_TRIGGER_GLOBAL_THREAD=y" zephyr/prj.conf >/dev/null
...
rg -n "500 ms|500ms" docs/AT_USAGE.md >/dev/null
